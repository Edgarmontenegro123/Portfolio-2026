...
export const ParticlesBackground = () => {
    ...
    useEffect(() => {
        ...
        const handleMouseEnterProjects = () => { isPaused = true; };
        const handleMouseLeaveProjects = () => { isPaused = false; };

        const projectsSection = document.getElementById('projects');
        if (projectsSection) {
            projectsSection.addEventListener('mouseenter', handleMouseEnterProjects);
            projectsSection.addEventListener('mouseleave', handleMouseLeaveProjects);
        }
        // ... (aquí mantienes los listeners de mousemove y mouseleave)
        ...
        class Particle {
            ...
            draw(ctx: CanvasRenderingContext2D) {
                // 🟢 Aquí ctx ya está asegurado por el guard del useEffect
        ...
        // 🟢 LA CORRECCIÓN CLAVE: Al encapsular connect dentro del useEffect,
        // ya hereda la validación de que 'ctx' y 'canvas' existen y no son null.
        const connect = () => {
            const isDark = document.documentElement.classList.contains('dark');
        ...
        return () => {
            ...
            if (projectsSection) {
                projectsSection.removeEventListener('mouseenter', handleMouseEnterProjects);
                projectsSection.removeEventListener('mouseleave', handleMouseLeaveProjects);
            }
        };
    }, []);
